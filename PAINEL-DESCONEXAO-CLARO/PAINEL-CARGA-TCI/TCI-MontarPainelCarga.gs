/**
 * PAINEL CARGA TCI — Miscelânia (ontem × hoje) + Modems
 *
 * Setup:
 ...
 *
 * Planilhas fonte (cópias):
 *   MATERIAIS — BASE MICELANEAS
 *   EQUIPES  — lista TT dos técnicos TCI
 */
function onOpen() {
...
/** Planilha painel TCI Carga (sua planilha v2) */
var PAINEL_ID_ = '1YAYbViLyvQoIEkeBizwOHSKMc9sxkE0Rl8L8bUGro9M';

/** IDs das cópias — edite em CONFIG se mudar */
var CFG_PADRAO_ = {
  MATERIAIS_ID: '1dxK-p-blCB8TBRAfCBRWhjXHNiYBpGExkFAQE9PQa4M',
  EQUIPES_ID: '1K1SzwTSSCri57o0U1A58EWT6cA4s0-XFjC5f5PyYR-A',
  ABA_MISC: 'BASE MICELANEAS',
  ABA_ALMOX: 'ALMOX SERIALIZADA',
  ABA_EQUIPES: 'Cópia de Página1',
...
function criarAbaInicio_(ss) {
  ...
  const passos = [
    ...
    ['3', 'PAINEL MODEMS → seriais por TT (após colar ALMOX)'],
    ['4', 'Amanhã repita o passo 1 para ver quem BAIXOU drop'],
    ['', 'Aba CONFIG = IDs das planilhas fonte (cópias)']
  ];
...
function preencherConfig_(sh) {
  const dados = [
    ...
    ['Técnicos', 'Somente TT listados na planilha EQUIPES'],
...
// ─── Equipe TCI (somente planilha EQUIPES, chave TT) ─────────────────────────
...
function carregarEquipeTci_(cfg) {
  ...
  if (last < 2) return { lista: [], porTt: {} };

  const dados = sh.getRange(1, 1, last, Math.max(sh.getLastColumn(), 6)).getValues();
  const header = dados[0].map(normalizarTexto_);
  const ixNome = indiceColuna_(header, ['nome do tecnico', 'nome do técnico', 'nome']);
  const ixTt = indiceColuna_(header, ['tt']);
  const ixArea = indiceColuna_(header, ['area atual', 'área atual', 'area']);
  const ixStatus = indiceColuna_(header, ['status', 'situação', 'situacao']);
  ...
  return { lista: lista, porTt: porTt };
}
...
function lerMiscFiltrada_(cfg, porTt) {
  ...
  const ix = {
    ...
    nome: indiceColuna_(header, ['armazem', 'armazém']),
    ...
    sub: indiceColuna_(header, ['subsegmento', '_subsegmento', 'sub grupo agregador'])
  };
  ...
  for (let i = 1; i < dados.length; i++) {
    ...
    if (ehOntModem_(grupo, sub, material)) continue;
...
function ehOntModem_(grupo, subsegmento, material) {
  ...
  if (m.indexOf('ONT CLIENTCO') === 0) return true;
  if (m.indexOf('ONT TIM') === 0) return true;
  if (m.indexOf('ONT G-') === 0) return true;
  if (m.indexOf('ONT 240') === 0) return true;
  if (m.indexOf('ONT VTAL') === 0) return true;
  return false;
}
...
function montarPainelMiscelania() {
  ...
  Object.keys(chaves).sort().forEach(function (chave) {
    ...
    const hoje = (snapshots.porData[dataHoje] && snapshots.porData[dataHoje][chave]) || null;
    const ontem = dataOntem && snapshots.porData[dataOntem]
      ? snapshots.porData[dataOntem][chave]
      : null;

    const saldoHoje = hoje ? hoje.saldo : 0;
    const saldoOntem = ontem ? ontem.saldo : 0;
    const ref = hoje || ontem;
...
function escreverResumoDrop_(sh, row, snapshots, dataOntem, dataHoje, porTt) {
  ...
  if (resumo.length) {
    ...
    sh.getRange('A' + cab + ':H' + fim).setBorder(
...
function colorirSituacao_(sh, rowIni, rowFim, colSit) {
  ...
  const cores = vals.map(function (r) {
  ...
  for (let i = 0; i < vals.length; i++) {
    if (cores[i][0]) sh.getRange(rowIni + i, colSit).setBackground(cores[i][0]);
  }
}

// ─── Painel modems (Almox × Equipe, por TT) ────────────────────────────────
...
function montarPainelModems() {
  ...
  titulo_(sh, 'A1:G1', 'PAINEL MODEMS TCI — CARGA POR TT (Almox serializados)');
  sh.getRange('A2:G2').merge()
    .setValue('Fonte: ALMOX SERIALIZADA · Somente TT da planilha EQUIPES · Atualizado: use após colagem 10h')
    .setFontStyle('italic').setHorizontalAlignment('center').setFontSize(10);

  cabecalhoTabela_(sh, 'A4:G4', [
    'TT', 'Técnico', 'Área', 'Qtd modems', 'Seriais', 'Tipo material', 'Status equipe'
  ]);
  ...
  modems.forEach(function (m) {
    if (!porTt[m.tt]) {
      porTt[m.tt] = {
        tt: m.tt,
        nome: m.nome,
        area: equipe.porTt[m.tt] ? equipe.porTt[m.tt].area : '',
        status: equipe.porTt[m.tt] ? equipe.porTt[m.tt].status : '',
        seriais: [],
        tipos: {}
      };
    }
  ...
  const linhas = equipe.lista.map(function (tec) {
    const g = porTt[tec.tt];
    if (!g) {
      return [tec.tt, tec.nome, tec.area, 0, '', '', tec.status];
    }
    const tipos = Object.keys(g.tipos).map(function (t) {
      return t + ' (' + g.tipos[t] + ')';
    }).join(', ');
    return [
      tec.tt,
  ...
  [90, 220, 80, 70, 400, 180, 80].forEach(function (w, i) { sh.setColumnWidth(i + 1, w); });
  sh.setFrozenRows(4);
  SpreadsheetApp.flush();
...
function lerModemsAlmox_(cfg, porTt) {
  ...
  const dados = sh.getRange(1, 1, last, sh.getLastColumn()).getValues();
  const header = dados[0].map(normalizarTexto_);
  const ixSerial = indiceColuna_(header, ['nº de série', 'n de serie', 'nº de serie', 'serial']);
  const ixTt = indiceColuna_(header, ['gestech', 'tt']);
  const ixNome = indiceColuna_(header, ['técnico', 'tecnico']);
  const ixTipo = indiceColuna_(header, ['tipo de material', 'texto breve material']);
...
/** getRange(linha, col, numLinhas, numCols) — 3º arg é QUANTIDADE, não linha final */
